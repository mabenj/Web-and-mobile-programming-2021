...
class App extends React.Component {
  ...
  updateGoodPercentage = () => {
    let n = this.state.good + this.state.neutral + this.state.bad;
    this.setState({ goodPercentage: n !== 0 ?(this.state.good / n * 100).toFixed(2) : 0 });
  };
    ...
    return (
      <div>
        ...
        <div className="buttons">
          ...
          <Button displayName="Neutraalia" handler={this.addFeedback('neutral')}></Button>
...
const Button = ({ displayName, handler, color }) => {
  return <button style={{backgroundColor: color}} onClick={handler}>{displayName}</button>
}
    ...
    <div>
      ...
      <table>     
        <tbody>
          <Statistic 
            displayName="Hyviä"
            displayValue={stats.good}>
          </Statistic>      
          <Statistic 
            displayName="Neutraaleja"
            displayValue={stats.neutral}>
          </Statistic>
          <Statistic 
            displayName="Kehnoja"
            displayValue={stats.bad}>
          </Statistic>
          <Statistic 
            displayName="Keskiarvo"
            displayValue={stats.average}>
          </Statistic>
          <Statistic 
...
const Statistic = ({displayName, displayValue, isPercentage}) => {
  return (
    <tr>
      ...
      <td>{displayValue}{isPercentage ? "%" : ""}</td>
    </tr>
  );
}
